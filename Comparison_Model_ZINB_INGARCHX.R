# ==============================================================================
#  COMPARISON MODEL: Zero-Inflated Negative Binomial INGARCHX(1,1) Model
#  (ZINB-INGARCHX) via Two-Phase Adaptive MCMC (Chen & So, 2006)
...
#
#  Model structure (ZINB):
#    P(Y_t = 0)   = rho + (1 - rho) * (r / (r + lambda_t))^r
#    P(Y_t = y)   = (1 - rho) * NB(y; r, lambda_t),  y > 0
#    lambda_t     = beta1 + beta2 * Y_{t-1} + beta3 * lambda_{t-1}
#                   + sum_i omega_i * X_i[t - b_i]          (if k >= 1)
#    Var(Y_t)     = lambda_t + lambda_t^2 / r  (NB quadratic variance)
#  Parameters: rho, beta1, beta2, beta3, [omega1, omega2], r, [b1, b2]
#  Note: No tail parameter phi (unlike ZIBNB-INGARCHX); serves as comparison model.
# ==============================================================================
...
# ==============================================================================
# SECTION 3: ZINB Log-Likelihood
# ==============================================================================
#  rho   : structural zero-inflation probability in (0, 1)
#  beta  : c(beta1, beta2, beta3) — INGARCHX conditional mean parameters
#  omega : vector length k — exogenous coefficients (> 0)
#  r     : NB dispersion parameter (> 0)
#  b     : integer vector length k — lag indices for exogenous variables
#
#  NB parameterisation: mean = lambda_t, p = r / (r + lambda_t)
#    P(Y = 0)  = p^r
#    P(Y = y)  = C(y+r-1, y) * p^r * (1-p)^y,   y > 0
# ==============================================================================
log_likelihood <- function(Y, rho, beta, omega, r, X_list, b, b0, k) {
  n        <- length(Y)
  log_like <- 0
  ...
  for (t in (b0 + 1):n) {
    ...
    p_t <- r / (r + lambda_t[t])           # P(success) for NB
    p_t <- min(max(p_t, 1e-10), 1 - 1e-10)
    
    # --- NB point mass at zero ---
    log_nb_0 <- r * log(p_t)              # log( (r/(r+lambda))^r )
    nb_0     <- exp(log_nb_0)
    
    if (Y[t] == 0) {
      # P(Y_t = 0) = rho + (1 - rho) * NB(0; r, lambda_t)
      mix_zero <- rho + (1 - rho) * nb_0
      mix_zero <- max(mix_zero, 1e-300)
      log_like <- log_like + log(mix_zero)
    } else {
      # P(Y_t = y) = (1 - rho) * NB(y; r, lambda_t),   y > 0
      log_nb_y <- lgamma(Y[t] + r) - lgamma(Y[t] + 1) - lgamma(r) +
        r * log(p_t) + Y[t] * log(1 - p_t)
      log_like <- log_like + log(1 - rho) + log_nb_y
    }
  }
  return(log_like)
}
...
run_mcmc_empirical <- function(Y, X_list, k, N = 20000, burn_in = 8000,
  ...
  # --- Parameter vector layout: [rho, beta(3), omega(k), r, b(k)] ---
  # Note: no phi in ZINB
  n_param         <- 1 + 3 + k + 1 + k
  ...
  idx_r     <- 4L + k + 1L
  idx_b     <- if (k > 0) (4L + k + 2L):(4L + 2L*k + 1L) else integer(0)
    ...
    if (k > 0) {
      for (bi_idx in 1:k) {
        ...
        lik_b  <- sapply(1:b0_mcmc, function(j) {
          ...
          b_cand[bi_idx]   <- j
          log_likelihood(Y, rho_current, beta_current, omega_current,
                         r_current, X_list, b_cand, b0_mcmc, k)
        })
        max_lik  <- max(lik_b)
  ...
  n_phase2   <- N - burn_in
  # Blocks: rho, beta, [omega], r  (no phi for ZINB)
  n_blocks   <- if (k > 0) 4L else 3L
...
cat("Running 4 ZINB-INGARCHX models on", ncores, "cores...\n")
t0 <- Sys.time()
...
summarize_empirical <- function(res, param_names, b_names = NULL) {
  samples <- res$samples
  k       <- res$k
  # ZINB continuous params: rho + beta(3) + omega(k) + r  (no phi)
  n_cont  <- 1 + 3 + k + 1
...
compute_geweke_IF <- function(res, param_names, model_label) {
  ...
  # Per-parameter acceptance rates (no phi block)
  ar <- res$accept_rates_phase2
  accept_vec <- c(
    ar$rho,
    rep(ar$beta, 3),                             # beta1, beta2, beta3 share block
...
compute_DIC <- function(samples, Y, X_list, b0, k,
                        idx_rho, idx_beta, idx_omega, idx_r, idx_b) {
...
compute_predicted <- function(res, Y, X_list, b0_mcmc = 3,
  ...
  for (t in (b0_mcmc + 1):n) {
    ...
    # ZINB marginal mean: E[Y_t] = (1 - rho) * lambda_t
    mu_t[t] <- (1 - rho_bar) * lambda_t[t]
    
    # ZINB marginal variance:
    #   Var[Y_t] = (1-rho)*lambda_t + (1-rho)*lambda_t^2/r + rho*(1-rho)*lambda_t^2
    var_t[t] <- (1 - rho_bar) * lambda_t[t] +
      (1 - rho_bar) * lambda_t[t]^2 / r_bar +
      rho_bar * (1 - rho_bar) * lambda_t[t]^2
  ...
  pdf(paste0("prediction_diagnostics_CenluzLepto_ZINB_", safe_label, ".pdf"),
      width = 12, height = 8)
  ...
  plot(time_seq, Y, type = "l", lty = 2, col = "blue",
       xlab = "", ylab = "Cases",
       main = paste0("Cenluz Lepto ZINB-INGARCHX — ", model_label),
       cex.main = 1.1, xaxt = "n")
  ...
  dev.off()
  cat("Prediction plot saved for Cenluz Lepto ZINB:", safe_label, "\n")
...
plot_empirical <- function(res, param_names, model_label) {
  ...
  pdf(paste0("traceplot_CenluzLepto_ZINB_", model_label, ".pdf"))
  par(mfrow = c(nr, 3))
  for (i in 1:n_cont) {
  ...
  pdf(paste0("acf_CenluzLepto_ZINB_", model_label, ".pdf"))
  par(mfrow = c(nr, 3))
  for (i in 1:n_cont) {
  ...
  cat("Plots saved for Cenluz Lepto ZINB", model_label, "\n")
}

# ==============================================================================
# SECTION 11: Run All Summaries, Diagnostics, Predictions
# ==============================================================================
# Parameter names: rho, beta1, beta2, beta3, [omega1, omega2], r  (no phi)
param_m1 <- c("rho", "beta1", "beta2", "beta3", "r")
param_m2 <- c("rho", "beta1", "beta2", "beta3", "omega1", "r")
param_m3 <- c("rho", "beta1", "beta2", "beta3", "omega1", "r")
param_m4 <- c("rho", "beta1", "beta2", "beta3", "omega1", "omega2", "r")
...
cat("\n========== Model 1 Summary (ZINB, No Exo) ==========\n")
print(summary_m1)
cat("\n========== Model 2 Summary (ZINB, Rainfall Only) ==========\n")
print(summary_m2)
cat("\n========== Model 3 Summary (ZINB, Max Temp Only) ==========\n")
print(summary_m3)
cat("\n========== Model 4 Summary (ZINB, Rain + Max Temp) ==========\n")
print(summary_m4)
...
# ==============================================================================
write.csv(summary_m1, "CenluzLepto_ZINB_result_model1.csv",      row.names = FALSE)
write.csv(summary_m2, "CenluzLepto_ZINB_result_model2.csv",      row.names = FALSE)
write.csv(summary_m3, "CenluzLepto_ZINB_result_model3.csv",      row.names = FALSE)
write.csv(summary_m4, "CenluzLepto_ZINB_result_model4.csv",      row.names = FALSE)
write.csv(dic_table,  "CenluzLepto_ZINB_DIC_comparison.csv",     row.names = FALSE)
write.csv(diag_m1,    "CenluzLepto_ZINB_diagnostics_model1.csv", row.names = FALSE)
write.csv(diag_m2,    "CenluzLepto_ZINB_diagnostics_model2.csv", row.names = FALSE)
write.csv(diag_m3,    "CenluzLepto_ZINB_diagnostics_model3.csv", row.names = FALSE)
write.csv(diag_m4,    "CenluzLepto_ZINB_diagnostics_model4.csv", row.names = FALSE)

# New: comprehensive diagnostics CSV (Geweke, IF, Bias, Rel.Bias, AcceptRate, RMSE)
write.csv(comprehensive_diag_all,
          "CenluzLepto_ZINB_comprehensive_diagnostics.csv",
          row.names = FALSE)

cat("\nAll Cenluz Lepto ZINB-INGARCHX results saved.\n")
