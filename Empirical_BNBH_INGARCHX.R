...
#
#  REVISION: Step sizes refactored as a named-list argument per model.
#            N increased to 120000 (100k posterior) for stable DIC.
#            Step sizes tuned for all four models.
#            omega unpacking fixed for variable-length vectors.
# ==============================================================================
...
#
#  ARGUMENT: step_sizes — a named list with elements:
...
#
#  N increased to 120000 (100k posterior) for stable E[D] and pD.
#  omega unpacking uses length-safe recycling.
# ==============================================================================
...
run_mcmc_empirical <- function(Y, X_list, k, N = 20000, burn_in = 8000,
  ...
  # --- Unpack step sizes — length-safe for variable-length omega ---
  step_size_alpha <- step_sizes$alpha
  step_size_beta  <- step_sizes$beta
  step_size_r     <- step_sizes$r
  step_size_phi   <- step_sizes$phi
  ...
  for (iter in 1:burn_in) {
    ...
    if (k > 0) {
      for (bi_idx in 1:k) {
        lik_b <- NULL; b_temp <- b; b[bi_idx] <- 0
...
#
#  All four models now use tuned step sizes based on posterior range diagnostics.
#  beta0 fix (0.05 -> 0.40) applied to all models — the same structural defect
#  existed in Models 1, 3, 4 as it did in Model 2.
#
#  Target acceptance rate in Phase 1 RWM: 23-44% per block.
#  If after re-run a block's acceptance rate is outside this range:
#    - acceptance too high (>60%): step is too small, increase it
#    - acceptance too low (<15%): step is too large, reduce it
# ==============================================================================

step_sizes_m1 <- list(
  # Model 1 — No Exo.
  # Same hurdle-INGARCH structure as M2; no exo means beta0 absorbs more
  # variation — posterior range ~2.0, same fix applies.
  # alpha0: range ~1.5 -> 0.28; alpha1: ~0.4 -> 0.08; alpha2: ~0.8 -> 0.18
  # beta0:  range ~2.0 -> 0.40; beta1:  ~0.45 -> 0.08; beta2: ~0.45 -> 0.12
...
step_sizes_m2 <- list(
  # Model 2 — Rainfall Only (best model, primary tuning reference).
  # alpha0: range ~1.5 -> 0.28 (~19% of range)
  # alpha1: decaying well, reduce slightly to 0.08 for precision
  # alpha2: range ~0.8 -> 0.18 (~23% of range)
  alpha = c(0.28, 0.08, 0.18),
  # beta0:  range ~2.0, OLD 0.05 = 2.5% of range -> PRIMARY FIX -> 0.40
  # beta1:  range ~0.45 -> 0.08 (~18% of range)
  # beta2:  range ~0.45, OLD 0.50 caused excess constraint rejections -> 0.12
  beta  = c(0.40, 0.08, 0.12),
  # r:   range ~8, ACF decays within lag 4 -> keep 0.80
  r     = 0.80,
  # phi: range ~10, ACF decays within lag 4 -> keep 0.60
  phi   = 0.60,
  # omega1: range ~1.5, slight tightening from 0.50 -> 0.45
  omega = 0.45
)

step_sizes_m3 <- list(
  # Model 3 — Mean Temp Only.
  # Temperature coefficient posterior range ~1.2 (weaker signal than rainfall ~1.5).
  # omega step 0.35 ~ 29% of range.
  alpha = c(0.28, 0.08, 0.18),
  beta  = c(0.40, 0.08, 0.12),
  r     = 0.80,
  phi   = 0.60,
...
step_sizes_m4 <- list(
  ...
  # omega is length-2: c(0.45, 0.35) matches rainfall range ~1.5 and
  # temperature range ~1.2 respectively.
  # Length-safe unpacking in run_mcmc_empirical handles this correctly.
  alpha = c(0.28, 0.08, 0.18),
  beta  = c(0.40, 0.08, 0.12),
  r     = 0.80,
  phi   = 0.60,
...
# ==============================================================================
# SECTION 10: In-Sample Prediction and Residual Diagnostics
# ==============================================================================
compute_predicted <- function(res, Y, X_list, b0_mcmc = 3,
  ...
  plot(time_seq, Y, type = "l", lty = 2, col = "blue",
       xlab = "", ylab = "Cases",
       main = paste0("Central Luzon Leptospirosis Data"),
       cex.main = 1.1, xaxt = "n")
...
# ==============================================================================
# SECTION 11: Traceplots and ACF Plots
# ==============================================================================
...
cat("\nAll CenLuz leptospirosis results saved.\n")
